...
- 上部＝ステータス盤（見るだけ）: 構成図 + キャラの表情 + Day / 信頼 / 費用 / 稼働状態の帯
- 下部＝作業エリア（操作する）: 設計 / 顧客 / 運用 の3タブ + 実行結果のコンソール
- キャラ画像は `chara_normal` / `chara_worry`（障害中）/ `chara_angry`（信頼35未満）を状態で自動切替
- ノードアイコンは VectorDrawable（ic_pc / ic_guest / ic_switch / ic_server / ic_cloud）、Firewall のみ提供された PNG（node_firewall）
